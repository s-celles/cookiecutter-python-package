...
def main():
    ...
    try:
        ...
        if "{{ cookiecutter.use_ruff }}" == "y":
            print("* Ruff: Ultra-fast Python linter and formatter that replaces multiple tools (flake8, black, isort)")

        if "{{ cookiecutter.use_mypy }}" == "y":
            print("* MyPy: Static type checker that helps catch bugs early and improves code documentation")

        if "{{ cookiecutter.use_pytest }}" == "y":
            print("* pytest: Modern testing framework with powerful features and fixtures")

        if "{{ cookiecutter.use_coverage }}" == "y":
            print("* Coverage: Measures test coverage to ensure your tests are comprehensive")

        if "{{ cookiecutter.use_pre_commit }}" == "y":
            print("* pre-commit: Runs checks before commits to maintain code quality automatically")

        if "{{ cookiecutter.use_bandit }}" == "y":
            print("* Bandit: Security linter that finds common security issues in Python code")

        if "{{ cookiecutter.use_safety }}" == "y":
            print("* Safety: Checks dependencies for known security vulnerabilities")

        if "{{ cookiecutter.use_github_actions }}" == "y":
            print("* GitHub Actions: Automated CI/CD pipelines for testing and deployment")

        if "{{ cookiecutter.use_dependabot }}" == "y":
            print("* Dependabot: Automated dependency updates to keep your project secure")
